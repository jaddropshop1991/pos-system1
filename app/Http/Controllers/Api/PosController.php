...
class PosController extends Controller
{
    ...
    public function getProduct($id){
        //get all products belonging to the category_id by the request category_id of this function send by the pointofsale.vue 
    ...
    public function orderDone(Request $request){
        ...
        //use insertGetId laravel method to get the last specific id data
    ...
    public function SearchOrderDate(Request $request){
        ...
        // //reformat the returned date by the request to match the format
    ...
    public function TodayDue(){
        $date = date('d/m/Y');
        $todaydue = DB::table('orders')->where('order_date',$date)
        ->sum('due');
    ...
    public function TodayExpense(){
        $date = date('d/m/Y');
        $expense = DB::table('expenses')->where('expenses_date',$date)
        ->sum('amount');
        return response()->json($expense);
    }

    public function Stockout(){
        $product = DB::table('products')->where('product_quantity','<','1' )
        ->get();
        return response()->json($product);
    }
